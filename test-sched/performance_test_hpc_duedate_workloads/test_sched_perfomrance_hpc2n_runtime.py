...
filename = "../HPCworkloads/HPC2N-2002-2.2-cln.swf"
...
left,mode,right = 0, 0.75, 1    # ~25%-jobs for mics
...
for i in xrange(0, NUM_EXPERIMENTS):  # 1e7
    ...
    for j in xrange(0, 16):
        ...
        task_file.write(str(tasks_state_runtimes[j]) + ","
            ...
            + str(tasks_state_duedate[j]) + ","   # for duedate values
            + str(tasks_state_req_runtimes[j]) + "\n")
    ...
    while model_submit_times[choose+num_tasks_state+j] - earliest_submit <= SECONDS_IN_A_DAY * SIM_NUM_DAYS:
        ...
        task_file.write(str(tasks_queue_runtimes[j]) + ","
            ...
            + str(tasks_queue_duedate[j]) + ","   # for duedate values
            + str(tasks_queue_req_runtimes[j]) + "\n")
        j = j+1
    task_file.close()
    choose = choose + num_tasks_state + j

    number_of_tasks = len(tasks_queue_runtimes) + len(tasks_state_runtimes)
    print("Experiment %d: the number of jobs = %d" % (i, number_of_tasks))

    _buffer = open("plot-temp.dat", "w+")
    subprocess.call(['./sched-simulator-runtime simgrid-xmls/plat_day.xml simgrid-xmls/deployment_hpc2n.xml -nj ' +
                     str(number_of_tasks)], shell=True, stdout=_buffer)
    subprocess.call(['./sched-simulator-runtime simgrid-xmls/plat_day.xml simgrid-xmls/deployment_hpc2n.xml -spt -nj ' +
                     str(number_of_tasks)], shell=True, stdout=_buffer)
    subprocess.call(['./sched-simulator-runtime simgrid-xmls/plat_day.xml simgrid-xmls/deployment_hpc2n.xml -lpt -nj ' +
                     str(number_of_tasks)], shell=True, stdout=_buffer)
    subprocess.call(['./sched-simulator-runtime simgrid-xmls/plat_day.xml simgrid-xmls/deployment_hpc2n.xml -wfp3 -nj ' +
                     str(number_of_tasks)], shell=True, stdout=_buffer)
    subprocess.call(['./sched-simulator-runtime simgrid-xmls/plat_day.xml simgrid-xmls/deployment_hpc2n.xml -unicef -nj ' +
                     str(number_of_tasks)], shell=True, stdout=_buffer)
    subprocess.call(['./sched-simulator-runtime simgrid-xmls/plat_day.xml simgrid-xmls/deployment_hpc2n.xml -edd -nj ' +
                     str(number_of_tasks)], shell=True, stdout=_buffer)
    # subprocess.call(['./sched-simulator-runtime simgrid-xmls/plat_day.xml simgrid-xmls/deployment_hpc2n.xml -easy -nj ' +
    #                  str(number_of_tasks)], shell=True, stdout=_buffer)
    subprocess.call(['./sched-simulator-runtime simgrid-xmls/plat_day.xml simgrid-xmls/deployment_hpc2n.xml -f1 -nj ' +
                     str(number_of_tasks)], shell=True, stdout=_buffer)
    subprocess.call(['./sched-simulator-runtime simgrid-xmls/plat_day.xml simgrid-xmls/deployment_hpc2n.xml -f2 -nj ' +
                     str(number_of_tasks)], shell=True, stdout=_buffer)
    subprocess.call(['./sched-simulator-runtime simgrid-xmls/plat_day.xml simgrid-xmls/deployment_hpc2n.xml -f3 -nj ' +
                     str(number_of_tasks)], shell=True, stdout=_buffer)
    subprocess.call(['./sched-simulator-runtime simgrid-xmls/plat_day.xml simgrid-xmls/deployment_hpc2n.xml -f4 -nj ' +
                     str(number_of_tasks)], shell=True, stdout=_buffer)
    _buffer.close()
...
# _test_buffer = open("./plot-slowdown/plot-slowdown-hpc2n-0-mics.dat", "w+")
# _test_buffer = open("./plot-lateness/plot-lateness-hpc2n-75-mics.dat", "w+")
_test_buffer = open("./plot-throughput/plot-throughput-hpc2n-25-mics.dat", "w+")
...
# show the end
# print('Boxplot saved in file ./plot-slowdown/figures/')
# print('Boxplot saved in file ./plot-lateness/figures/')
print('Boxplot saved in file ./plot-throughput/figures/')
