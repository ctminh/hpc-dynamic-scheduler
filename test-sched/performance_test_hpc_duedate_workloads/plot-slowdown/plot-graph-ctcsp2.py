...
# file input to read
filename = "./plot-slowdown-ctcsp2-0-mics.dat"
...
plt.annotate(output_fcfs, xy=(xticks[0], 490), xytext=(xticks[0]-x_offset[0], y_low[0]),
                    arrowprops=dict(facecolor='black', shrink=0.05),fontsize=24)
output_spt = '%.1f' % (outliers[1,0])
plt.annotate(output_spt, xy=(xticks[1], 490), xytext=(xticks[1]-x_offset[1], y_low[1]),
                    arrowprops=dict(facecolor='black', shrink=0.05),fontsize=24)
output_lpt = '%.1f' % (outliers[2,0])
plt.annotate(output_lpt, xy=(xticks[2], 490), xytext=(xticks[2]-x_offset[2], y_low[2]),
                    arrowprops=dict(facecolor='black', shrink=0.05),fontsize=24)
output_wfp3 = '%.1f' % (outliers[3,0])
plt.annotate(output_wfp3, xy=(xticks[3], 490), xytext=(xticks[3]-x_offset[3], y_low[3]),
                    arrowprops=dict(facecolor='black', shrink=0.05),fontsize=24)
...
plt.savefig('figures/slowdown-ctcsp2-0-mic.pdf', format='pdf', dpi=1000, bbox_inches='tight')
print('Boxplot saved in file slowdown-ctcsp2-0-mic.pdf')
