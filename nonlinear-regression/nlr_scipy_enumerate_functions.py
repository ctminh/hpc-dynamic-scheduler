...
def main():
    ...
    # print("runtimes \t nodes \t submits \t mics \t duedates \t train_labels")
    # for i in xrange(0, len(runtimes)):
    #     print("%f \t %d \t %d \t\t %d \t %d \t\t %f" % (runtimes[i], nodes[i], submits[i], mics[i], duedates[i], train_labels[i]))
    ...
    permutations = set(list(itertools.product([0,1,2,3], repeat=NUM_FEATURES-2)))
    ...
    for op in range(0, 3):
        for append_op1 in range(0, 3):
            ...
            for perm in permutations:
                ...
                # print("[loop %d]: op = %s, append_op1 = %s, append_op2 = %s" % (idx, op_labels[op], op_labels[append_op1], op_labels[append_op2]))
                ...
                def f(x, p1, p2, p3):
                    r, n, s, m, d = x
                    _f1 = operators[c[0]]((r, n, s, m, d), functions[c[2]], functions[c[3]], p1, p2)
                    _f2 = append3_operators[c[1]]((r, n, s, m, d), _f1, functions[c[4]], p3)
                    # return append4_operators[c[2]]((r, n, s, m, d), _f2, functions[c[5]], p4)
                    return _f2
            
                popt, pcov = sp.curve_fit(f, (runtimes, nodes, submits, mics, duedates), train_labels, p0 = [0.7,0.7,0.7], sigma=w, absolute_sigma=True)
                ...
                for i in range(0, len(train_labels)):
                    f_values = f((runtimes[i], nodes[i], submits[i], mics[i], duedates[i]), p1, p2, p3)
    ...
    for i in range(0, len(all_c)):
        ...
        print("duedate * [(%.10f x %s(runtime)) %s (%.10f x %s(#cores)) + mics ] %s (%.10f x %s(submit)),fitness=%.7f" % (all_popt[max_score_index][0],f_labels[all_c[max_score_index][2]],
            op_labels[all_c[max_score_index][0]], all_popt[max_score_index][1], f_labels[all_c[max_score_index][3]],
            op_labels[all_c[max_score_index][1]], all_popt[max_score_index][2],f_labels[all_c[max_score_index][4]],
